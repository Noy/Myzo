...
func bulkTransactionsRequest(auth *Myzo, daysAgo,before int, expandBy string) *TransactionsResponse {
	r, _ := auth.transactionResponseHandler(true, daysAgo, before, expandBy, "")
	return r
}
...
func (auth *Myzo) GetAllTransactions(daysAgo, before int, expandBy string) []Transaction {
	return bulkTransactionsRequest(auth, daysAgo, before, expandBy).Transactions
}
...
func (auth *Myzo) GetAllMerchants(daysAgo, before int) []Merchant {
	var merchants []Merchant
	for _, t := range bulkTransactionsRequest(auth, daysAgo, before, "merchant").Transactions {
